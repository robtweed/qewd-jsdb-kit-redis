...
instantiate a QEWD-JSDB *Document Node Object* for a Document (ie a Redis Global) named *jsdbDom*,
...
As you've seen when you looked in Redis, the QEWD-JSdb DOM is persistent, being stored
...
So you've now disconnected from Redis.
...
methods directly access the Redis database when they're invoked, so they always return what's
...
as a Proxy Object which accesses the Redis data every time one of its properties or methods is
...
QEWD-JSdb storage (ie from the Redis Global):
...
parsed XML as a persistent DOM in Redis.
...
You'll find an 
[example XML document in this repository](https://github.com/robtweed/qewd-starter-kit-redis/blob/master/example.xml).  Download it and put it into your QEWD system's
installation directory:

- If you're using a Native installation of QEWD and Redis:
...
actual DOM data it is handling is actually stored in your Redis database!
...
a look at the 
[list of examples]((https://github.com/robtweed/qewd-starter-kit-redis/blob/master/xpath_query_examples.txt)
...
*getDocument()* methods to save JSON to a Redis Global and retrieve it again.  Why, then,
...
use much less Redis storage than the DOM for representing the JSON.

However, there are two potential benefits of using the DOM for JSON:
...
I hope you agree, it's a very powerful technology, and a great use of Global
Storage in Redis.  For XML Documents, its benefits are clear.  However, the trick is to
